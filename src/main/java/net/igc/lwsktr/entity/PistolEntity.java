...
public class PistolEntity extends AbstractArrow implements ItemSupplier {
	...
	public static PistolEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("lwsk_tr:shootsgun9mm")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}
	...
	public static PistolEntity shoot(LivingEntity entity, LivingEntity target) {
		...
		entity.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("lwsk_tr:shootsgun9mm")), SoundSource.PLAYERS, 1, 1f / (new Random().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
